...
class Analysis:
    ...
    def __init__(self, dnn, config=None):
        ...
        self.workload_dir = os.path.join(
            self.paths.get("workload_root", "./workloads"), self.DNN
        )
        ...
        if os.path.exists(self.workload_dir):
            layers = [
                f.split(".")[0]
                for f in os.listdir(self.workload_dir)
        ...
        for layer in self.layers:
            ...
            with open(layer_path, "r") as file:
                ...
                if instance_line:
                    ...
                    col = 12 * 128
                    num = max(
                        math.ceil(requirement_row / row),
                        math.ceil(requirement_col / col),
                    )
    ...
    def cal_weight_num(self):
        ...
        for layer in self.layers:
            ...
            if result.get("R") is not None:
                self.weight_num.append(
                    int(result["C"] * result["R"] * result["S"] * result["M"] * 16)
                )
            else:
                    ...
                    for level in levels:
                        if "=== dummy_top ===" in level:
                            inputs_match = re.search(
                                r"Inputs:.*?Address generations \(per-cluster\)\s*:\s*([\d.eE+-]+)",
                                level,
                                re.DOTALL,
                            )
                            outputs_match = re.search(
                                r"Outputs:.*?Address generations \(per-cluster\)\s*:\s*([\d.eE+-]+)",
                                level,
                                re.DOTALL,
                            )
                            weights_match = re.search(
                                r"Weights:.*?Address generations \(per-cluster\)\s*:\s*([\d.eE+-]+)",
                                level,
                                re.DOTALL,
                            )
                            ...
                            results.append(
                                {
                                    "inputs": (
                                        float(inputs_match.group(1))
                                        if inputs_match
                                        else None
                                    ),
                                    "outputs": (
                                        float(outputs_match.group(1))
                                        if outputs_match
                                        else None
                                    ),
                                    "weights": (
        ...
        with open(stats_txt, "r") as file:
            for line in file:
                if "Computes" in line:
                    ...
                    if match:
                        computes = int(match.group(1))

                match = re.match(
                    rf"\s*{re.escape(component_name)}\s*=\s*([0-9eE\+\-\.]+)", line
                )
                if match:
    ...
    def extract_cim_write_energy(self, file_path):
        ...
        try:
            ...
            for table in data["ERT"]["tables"]:
                if "name" in table and table["name"].startswith(
                    "system_top_level.cim_unit"
                ):
                    if "actions" in table:
                        for action in table["actions"]:
                            if (
                                "name" in action
                                and action["name"] == "write"
    ...
    def extract_vector_access_by_module(
        self, file_path, target_op, module_name="cim_unit"
    ):
        stats_txt = os.path.join(file_path, "timeloop-mapper.stats.txt")
        try:
            with open(stats_txt, "r") as file:
            ...
            vector_pattern = re.compile(
                rf"vector access\s+:\s+([\d\.e\+\-]+)\s+op_name:\s*{re.escape(target_op)}",
                re.IGNORECASE,
            )
