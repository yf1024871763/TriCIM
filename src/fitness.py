...
class FitnessEvaluator:
    """Fitness/objective callbacks used by BO and engine-side evaluation."""
    ...
    def get_noc_weight_delay(self, data_size, tiles, is_multicast=True):
        ...
        total_nodes = mesh_dim * mesh_dim

        base_source_injection_rate = 1.0

        if is_multicast:
            dynamic_injection_rate = base_source_injection_rate / total_nodes
            serialization_delay = data_size / width
        else:
            dynamic_injection_rate = min(
                0.95, (base_source_injection_rate * tiles) / total_nodes
            )
            serialization_delay = (data_size * tiles) / width

        dynamic_injection_rate = max(0.0001, dynamic_injection_rate)

        if hasattr(self.engine, "booksim"):
            noc_result = self.engine.booksim.run_simulation(
                mesh_dim=mesh_dim,
                traffic_pattern="uniform",
                injection_rate=dynamic_injection_rate,
            )
            base_latency = noc_result.get("base_latency", 0.0)

            avg_hops = noc_result.get("avg_hops", 0.0)
            if avg_hops > 0:
                router_delay_per_hop = base_latency / avg_hops
                max_hops = (mesh_dim - 1) * 2
                base_latency += router_delay_per_hop * max_hops * 0.5
        else:
            base_latency = 0.0
    ...
    def cnn_fitness_callback(self, individual):
        ...
        for i in range(len(self.layers) - 1):
            logging.debug(
                f"Processing Pipeline: {self.layers[i]} -> {self.layers[i+1]} | Maxpool: {self.pipeline_analyzer.maxpool[i]} | FC: {self.pipeline_analyzer.fc[i+1]}"
            )
        ...
        outputs = [
            2 * d[0]["outputs"] if d and len(d) > 0 else 0 for d in pipeline_access
        ]
        ...
        for i in range(len(self.layers)):
            ...
            cim_write_energy = self.analyzer.extract_cim_write_energy(
                output_path_pipeline[i]
            )
            ...
            base_energy = self.analyzer.get_total_energy(
                output_path_pipeline[i] + "/timeloop-mapper.stats.txt"
            )
            adjusted_energy = base_energy - (
                cim_utilized * cim_write_energy * vec_access / 1e6
            )
    ...
    def transformer_fitness_callback(self, individual):
        ...
        try:
            ...
            total_tile = sum(full_alloc) + (head_num - 1) * (
                full_alloc[a_idx] + full_alloc[z0_idx]
            )
        ...
        for i in range(block_num):
            ...
            if i == 0:
                ...
                for proj in Projection:
                    ...
                    _, cur_ts, _, last_t, Projection_dataspace[proj] = (
                        self.pipeline_analyzer.parse_dataspace(
                            output_path_pipeline[idx],
                            workload[idx],
                ...
                for proj in Projection:
                    ...
                    (
                        ...
                        cal_time,
                        next_st,
                        Projection_dataspace[proj],
                    ) = self.pipeline_analyzer.pipeline_analysis(
                        cur_ts,
                        0,
                        0,
                        Input_dataspace=FFN_dataspace.get("FFN2", []),
                        Output_dataspace=Projection_dataspace[proj],
                        transformer=True,
                    )
                    cal_time_list.append(cal_time)
                    bubble_list.append(bubble)
                    ...
                    # --- Attention Phase ---
            Attention_dataspace = {
                key: [[] for _ in range(head_num)] for key in Mh_attention
            }
            ...
            for attn in Mh_attention:
                ...
                for j in range(1):
                    if attn == "A":
                        _, cur_ts, _, last_t, Attention_dataspace[attn][j] = (
                            self.pipeline_analyzer.parse_dataspace(
                                output_path_pipeline[idx],
                                workload[idx],
            ...
            for j in range(1):
                (
                    ...
                    cal_time,
                    next_st,
                    Attention_dataspace["Z0"][j],
                ) = self.pipeline_analyzer.pipeline_analysis(
                    time_scale[4],
                    0,
                    0,
                    Input_dataspace=Attention_dataspace["A"][j],
                    Weight_dataspace=Projection_dataspace["V"],
                    Output_dataspace=Attention_dataspace["Z0"][j],
                    transformer=True,
                    attention=True,
                    transpose=True,
                )
            ...
            for ffn in FNN:
                ...
                _, cur_ts, _, last_t, FFN_dataspace[ffn] = (
                    self.pipeline_analyzer.parse_dataspace(
            ...
            actual_time, _, _, _, _, bubble, cal_time, next_st, FFN_dataspace["Z1"] = (
                self.pipeline_analyzer.pipeline_analysis(
                    time_scale[5],
                    0,
                    0,
                    Input_dataspace=Attention_dataspace["Z0"][0],
            ...
            if idx == 0:
                for batch in range(int(batch_size)):
                    ...
                    if batch == 0:
                        ...
                        _, cur_time_scale, _, last_time, Projection_dataspace["Q"] = (
                            self.pipeline_analyzer.parse_dataspace(
                        ...
                        _, cur_time_scale, _, last_time, Projection_dataspace["K"] = (
                            self.pipeline_analyzer.parse_dataspace(
                                output_path_pipeline[self.layers.index("Q")],
                                workload[self.layers.index("Q")],
                        ...
                        _, cur_time_scale, _, last_time, Projection_dataspace["V"] = (
                            self.pipeline_analyzer.parse_dataspace(
                                output_path_pipeline[self.layers.index("Q")],
                                workload[self.layers.index("Q")],
                ...
                for batch in range(int(batch_size)):
                    ...
                    if batch == 0:
                        ...
                        if layer_idx == 0:
                            ...
                            Q_start_time = max(
                                weight_update_start, start_time, end_time["Q"]
                            )
                        ...
                        (
                            ...
                            cal_time,
                            next_start_time,
                            Projection_dataspace["Q"],
                        ) = self.pipeline_analyzer.pipeline_analysis(
                            time_scale[5],
                            0,
                            0,
                            Input_dataspace=FFN_dataspace["FFN2"],
                            Output_dataspace=Projection_dataspace["Q"],
                            transformer=True,
                        )
                        current_batch_cal.append(cal_time)
                        current_batch_bubble.append(bubble)
                        ...
                        (
                            ...
                            cal_time,
                            next_start_time,
                            Projection_dataspace["K"],
                        ) = self.pipeline_analyzer.pipeline_analysis(
                            time_scale[6],
                            0,
                            0,
                            Input_dataspace=FFN_dataspace["FFN2"],
                            Output_dataspace=Projection_dataspace["K"],
                            transformer=True,
                        )
                        current_batch_cal.append(cal_time)
                        current_batch_bubble.append(bubble)
                        ...
                        (
                            ...
                            cal_time,
                            next_start_time,
                            Projection_dataspace["V"],
                        ) = self.pipeline_analyzer.pipeline_analysis(
                            time_scale[7],
                            0,
                            0,
                            Input_dataspace=FFN_dataspace["FFN2"],
                            Output_dataspace=Projection_dataspace["V"],
                            transformer=True,
                        )
                        current_batch_cal.append(cal_time)
                        current_batch_bubble.append(bubble)
    ...
    def cnn_multi_layer_fitness_callback(self, individual):
        ...
        logging.info(
            f"Evaluating Index= {individual[0]} | Groups: {workload_group} | Tiles: {tile_allocation}"
        )
        ...
        for i, layer in enumerate(self.layers):
            if "R" in workload[i].keys() or "S" in workload[i].keys():
                ...
                workload_size.append(
                    workload[i]["C"] * workload[i]["M"] * self.hw.get("precision", 16)
                )
            ...
            for batch in range(batch_size):
                ...
                if batch == 0:
                    if len(layers) == 1:
                        ...
                        real_delay = get_noc_weight_delay(
                            weight_access_group[k], group_tiles
                        )
                        ...
                        start_time = max(
                            weight_update_start,
                            (
                                end_time_dict.get(self.layers[idx - 1], 0)
                                if idx > 0
                                else 0
                            ),
                        )
                        _, cur_time_scale, _, last_time, Dataspace[batch][layers[0]] = (
                            self.pipeline_analyzer.parse_dataspace(
                                output_path_pipeline[idx],
                                workload[idx],
                                self.pipeline_analyzer.fc[idx],
                                start_time,
                            )
                        )

                        start_time_dict[batch][layers[0]] = end_time_dict.get(
                            layers[0], 0
                        )
                        current_batch_cal.append((start_time, last_time))
                        current_batch_bubble.append({})
                        ...
                        if k == 0:
                            for i, j in enumerate(layers):
                                ...
                                (
                                    ...
                                    output_path_pipeline[idx],
                                    workload[idx],
                                    fc=self.pipeline_analyzer.fc[idx],
                                    start_time=start_time,
                                )
                            ...
                            for i, j in enumerate(layers):
                                ...
                                (
                                    ...
                                    cal_time,
                                    _,
                                    Dataspace[batch][layers[i]],
                                ) = self.pipeline_analyzer.pipeline_analysis(
                                    time_scale[i],
                                    maxpooling,
                                    self.pipeline_analyzer.fc[idx],
                                    Input_dataspace=copy.deepcopy(
                                        Dataspace[batch][layers[i - 1]]
                                    ),
                            ...
                            last_layer_tile = [
                                tile_allocation[self.layers.index(x)]
                                for x in last_group
                            ]
                            available_tile = self.hw["tile_num"] - sum(last_layer_tile)
                            current_layer_tile = [
                                tile_allocation[self.layers.index(x)] for x in layers
                            ]
                            layer_p = 0
                            ...
                            for i, j in enumerate(layers):
                                ...
                                while available_tile < current_layer_tile[i]:
                                    ...
                                    weight_update_start = max(
                                        weight_update_start,
                                        end_time_dict[last_group[layer_p]],
                                    )
                            ...
                            for i, j in enumerate(layers):
                                ...
                                (
                                    ...
                                    self.pipeline_analyzer.fc[idx],
                                    Input_dataspace=copy.deepcopy(
                                        Dataspace[batch][self.layers[idx - 1]]
                                    ),
                                    Output_dataspace=copy.deepcopy(
                                        Dataspace[batch][layers[i]]
                                    ),
                    ...
                    if len(layers) == 1:
                        ...
                        start_time = max(
                            (
                                start_time_dict[batch].get(self.layers[idx - 1], 0)
                                if idx > 0
                                else 0
                            ),
                            end_time_dict[self.layers[idx]],
                        )

                        _, _, _, last_time, Dataspace[batch][layers[0]] = (
                            self.pipeline_analyzer.parse_dataspace(
                                output_path_pipeline[idx],
                                workload[idx],
                                self.pipeline_analyzer.fc[idx],
        ...
        outputs = [
            d[0]["outputs"] if d and len(d) > 0 else 0 for d in dataspace_results
        ]
        ...
        total_energy_pipeline = (
            sum(energy_pipeline)
            + (sum(weight_access_group) + pipeline_access * batch_size)
            * 112.54
            / 8
            / 1e6
        )
    ...
    def multi_group_transformer_fitness_callback(
        ...
        """
        Evaluate grouped transformer allocations with:
        - dynamic tile reclaiming,
        - NoC-aware weight transfer overlap,
        - block/batch scheduling constraints,
        - latency as the optimization objective.
        """
        import logging
        import math
        import copy
        ...
        if hasattr(individual, "flatten"):
            alloc_raw = individual.flatten().tolist()
        else:
            alloc_raw = (
                individual[0]
                if isinstance(individual[0], (list, tuple))
                else individual
            )

        full_alloc_raw = [alloc_raw[var_idx] for var_idx in var_map]

        hw_total_tiles = self.hw.get("tile_num", 1344)
        head_num = self.model.get("head_num", 1)
        batch_size = int(self.model.get("batch_size", 1))
        block_num = int(self.model.get("block", 1))
        default_macros = self.hw.get("macro_num", 16)

        tile_allocation = [1] * len(layer_names)
        macro_allocation = [default_macros] * len(layer_names)

        for i, layer_name_full in enumerate(layer_names):
        ...
        inputs = [d[0]["inputs"] if d and len(d) > 0 else 0 for d in pipeline_access]
        outputs = [d[0]["outputs"] if d and len(d) > 0 else 0 for d in pipeline_access]
        ...
        end_time_dict = [{layer: 0 for layer in layer_names} for _ in range(batch_size)]
        batch_end_time = [0 for _ in range(batch_size)]
        Dataspace = [{layer: [] for layer in layer_names} for _ in range(batch_size)]

        hw_available_tiles = hw_total_tiles
        weight_noc_ready_time = 0
        active_ops = []  # [(finish_time, allocated_tiles, layer_name)]

        def get_req_tiles(l_idx):
            t = tile_allocation[l_idx]
            name = layer_names[l_idx].split(".")[0]
        ...
        for b_idx in range(block_num):
            for stage_idx, group in enumerate(grouped_indices):
                ...
                for layer_idx in group:
                    ...
                    real_delay = self.get_noc_weight_delay(
                    ...
                    for batch in range(batch_size):
                        ...
                        if curr_name in ["Q", "K", "V"] and b_idx > 0:
                            if "FFN2" not in current_stage_ops:
                                should_pipeline = False
                                in_ds = None
                        if should_pipeline:
                            (
            ...
            idx_Q = layer_names.index("Q") if "Q" in layer_names else 0
            total_energy_pipeline += (
        ...
        for i in range(len(layer_names)):
            ...
            energy_pipeline.append(base_energy - (cim_util_pipe * cim_write_pipe))
            write_weight_energy_pipeline.append(cim_util_pipe * cim_write_pipe)

        Mh_attention = ["A", "Z0"]
        for i in Mh_attention:
            if i in layer_names:
                idx_i = layer_names.index(i)
        ...
        ) * block_num + weight_access_energy

        if actual_time == 0:
            return 1e15

            # logging.info(f"[BO EVAL] Latency: {actual_time} | Pipeline Energy: {total_energy_pipeline:.4f}")
        return actual_time
