...
def plot_bubble(save_path, layers_cal, layers_bubble, labels=None, actual_time=0):
    ...
    # Data validation
    assert len(layers_cal) == len(layers_bubble), "Number of layers in computation time and bubble time must match"
    if labels is None:
    ...
    for bar in bars:
        ...
        plt.text(bar.get_x() + bar.get_width() / 2, height,
                f'{height:.2f}%', ha='center', va='bottom', fontsize=14)  # Add percentage symbol
